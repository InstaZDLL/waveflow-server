...
- **Auth shim today, JWT in 1.d.** `middleware::require_user_id` reads `X-User-Id` from the request header and rejects 401 if missing / non-positive / unparseable. It does NOT round-trip to the DB to check the user exists — that's caught at insert time by the `profile.user_id` FK. When 1.d lands, swap the shim's body for JWT verification and emit the same `UserId` extension; handlers don't need to change.
