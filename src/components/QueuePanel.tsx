    ...
    <>
      ...
      >
        <header className="flex flex-shrink-0 items-center justify-between border-b border-[var(--line)] px-4 py-3">
          <h2 className="text-base font-semibold text-[var(--sea-ink)]">Queue</h2>
          <button
            ref={closeButtonRef}
            type="button"
            onClick={onClose}
            aria-label="Close queue"
            className="flex h-9 w-9 items-center justify-center rounded-full text-[var(--sea-ink)] transition hover:bg-[var(--link-bg-hover)]"
          >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          ...
          <section className="mb-6">
            ...
            {player.queue.length === 0 ? (
              <p className="text-sm text-[var(--sea-ink-soft)]">The queue is empty.</p>
            ) : (
...
function NowPlayingRow({ current }: { current: PlayingTrack }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-[var(--line)] bg-[var(--chip-bg)] px-3 py-2.5">
      <div
  ...
  return (
    <li>
      <button
        ...
        className="flex w-full items-center gap-3 rounded-lg px-2 py-1.5 text-left transition hover:bg-[var(--link-bg-hover)]"
      >
        <span
        ...
        <span className="min-w-0 flex-1">
          <span className="block truncate text-sm text-[var(--sea-ink)]">{entry.title}</span>
          {entry.artist && (
  ...
  return (
    <li className="flex items-center gap-3 rounded-lg px-2 py-1.5 opacity-70">
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm text-[var(--sea-ink)]">{entry.title}</span>
        {entry.artist && (
          <span className="block truncate text-xs text-[var(--sea-ink-soft)]">{entry.artist}</span>
        )}
      </span>
      <span className="text-xs tabular-nums text-[var(--sea-ink-soft)]">
        {formatDuration(entry.durationMs)}
