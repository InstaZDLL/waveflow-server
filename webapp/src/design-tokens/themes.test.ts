...
describe("THEME_PRESETS", () => {
  ...
  it("uses a full 11-shade scale for every accent", () => {
    ...
    for (const preset of THEME_PRESETS) {
      for (const shade of shades) {
        expect(preset.accent[shade as keyof typeof preset.accent]).toMatch(
          /^oklch\(/,
        );
      }
    }
  ...
  it("reuses palettes from ACCENT_PALETTES (no orphan inline scales)", () => {
    ...
    const known = Object.values(ACCENT_PALETTES) as ReadonlyArray<
      typeof ACCENT_PALETTES.EMERALD
    >;
    for (const preset of THEME_PRESETS) {
  ...
  it("every `pair` points at a real preset with the opposite mode", () => {
    ...
    for (const preset of THEME_PRESETS) {
      ...
      expect(
        mirror,
        `pair "${preset.pair}" referenced by "${preset.id}"`,
      ).toBeDefined();
  ...
  it("one-off presets without a pair are documented (OLED, Neon)", () => {
    ...
    const unpaired = THEME_PRESETS.filter((t) => t.pair === undefined).map(
      (t) => t.id,
    );
