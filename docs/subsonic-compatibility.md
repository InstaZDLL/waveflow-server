...
> **Juliet's row is dated 2026-08-31 and names the model `main` serves; the
> other four are dated earlier and do not.** On
...
> one `main` serves. Juliet was replayed on 2026-08-31 **against the five fields
> themselves**, on a catalogue built to carry each of the three array shapes —
> several values, exactly one, none — so its row is the first evidence that the
> 2026-08-24 change is safe on a real client. Symfonium, Feishin and DSub have
> not been replayed since, and §4 stays open for them.
...
| Juliet | 1.5 (iOS 26.6) | pass | limit | pass | pass | Re-run 2026-08-31 against the current model, on a physical iPhone through the HTTPS tunnel. **293 client requests, every one answered 200 or 206** — 265 and 28. It is the first row taken against the five album fields added on 2026-08-24, and the catalogue was built to make them fail if they were going to: three albums carrying **several** values of `recordLabels`, `releaseTypes` and `discTitles`, **exactly one** of each, and **none**. The single-element album is the shape that broke Symfonium at the handshake once, an array serialised as a bare object; it renders as `[{"name": "Solo Imprint"}]` and Juliet opened it and played its track twenty times. All three were opened and played, the two-disc album on both discs, and **no song carried any of the three** — the `537e011` restriction holding in front of a strict-decoding client. Also exercised: `search3`, `getGenres`, `getRandomSongs`, `getSimilarSongs2`, both lyrics methods, 6 stars and 1 unstar, 48 scrobbles, 39 saved play queues, a playlist created and updated that is in the database with its six tracks, and 28 partial-content reads on a seektable-less FLAC. **The artist split is offered and not consumed**, which is why this row reads `limit` where 2026-08-23 read `pass`: for an album credited to two artists the server answers `artists[]` as two entities with distinct ids, on the album and on the song, while the legacy `artist` field carries the joined display string — and that string is what Juliet shows. The index holds three artists and none with a semicolon. Nothing changed in the server between the two runs; the earlier row filed the same observation as client-side without saying what the server had offered, and this one asked. **Two limits of the method, stated rather than papered over**: the run went straight through `cloudflared` with no logging proxy, so the 28 range reads are countable and their byte offsets are not — the trace records path and status, never headers or query strings, which is also why the client version could not be recovered from the log — the operator confirmed it unchanged at 1.5, and the iOS build is carried forward from the previous row rather than re-checked. And the binary was a debug build; the release finished mid-run and was deliberately not swapped in, since changing the thing under test is not a validation. |
