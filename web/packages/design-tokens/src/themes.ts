...
import { AMBER, EMERALD, FUCHSIA, INDIGO, ROSE, SKY, VIOLET, type AccentPalette } from './palettes'
...
export function findTheme(id: string | null | undefined): ThemePreset {
  ...
  if (!def) {
    ...
    throw new Error(`design-tokens: default theme "${DEFAULT_THEME_ID}" missing from THEME_PRESETS`)
  }
  if (!id) return def
