...
export function applyTheme(theme: ThemePreset): void {
  ...
  root.style.setProperty('--color-surface-dark', theme.surfaceDark ?? DEFAULT_SURFACE_DARK)
  root.style.setProperty(
    '--color-surface-dark-elevated',
    theme.surfaceDarkElevated ?? DEFAULT_SURFACE_DARK_ELEVATED,
  )
  root.style.setProperty('--color-surface-light', theme.surfaceLight ?? DEFAULT_SURFACE_LIGHT)
  root.style.setProperty(
...
export function themeCssDeclarations(theme: ThemePreset): string {
  ...
  lines.push(`--ambient-bg: ${theme.ambient ?? (theme.mode === 'dark' ? '#121212' : '#ffffff')};`)
  lines.push(`--color-surface-dark: ${theme.surfaceDark ?? DEFAULT_SURFACE_DARK};`)
  lines.push(
    `--color-surface-dark-elevated: ${theme.surfaceDarkElevated ?? DEFAULT_SURFACE_DARK_ELEVATED};`,
  )
  lines.push(`--color-surface-light: ${theme.surfaceLight ?? DEFAULT_SURFACE_LIGHT};`)
  lines.push(
