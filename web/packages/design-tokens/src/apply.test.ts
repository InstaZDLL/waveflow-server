...
describe('applyTheme', () => {
  ...
  it('removes the dark class when switching to a light preset', () => {
    ...
    expect(document.documentElement.getAttribute('data-theme')).toBe('lavender-light')
  })
  ...
  it('clears overrides when going back to the default preset', () => {
    ...
    applyTheme(findTheme('lavender'))
    expect(document.documentElement.style.getPropertyValue('--color-surface-dark')).toBe('#15101e')
    applyTheme(findTheme('default-dark'))
    expect(document.documentElement.style.getPropertyValue('--color-surface-dark')).toBe('#121212')
  })
})
